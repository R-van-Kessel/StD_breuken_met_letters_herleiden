...
function generateAlgebraicFractionQuestion() {
  // Meer kans op een bepaald type door het vaker in de lijst te zetten
  let types = [
    'different_vars',   // 18xy/24xz = 3y/4z
    'simple_cancel',    // 12ab/4b = 3a
    'add_term',         // 8x/4 + 2x = 4x
    'subtract_frac',    // 7/3a - 2/a = 1/3a
    'factor_out'        // (2p+2q)/(4p+4q) = 1/2
  ];
  
  // Hulpfunctie ggd - wordt door alle types gebruikt
  ...
  if (type === 'different_vars') {
    // 18xy/24xz = 3y/4z
    // num en den zijn beide veelvouden van ggd
    let ggd = floor(random(2, 5));
    let num = ggd * floor(random(2, 5));
    let den = ggd * floor(random(2, 5));
    
    // Zorg dat num en den niet gelijk zijn (anders is antwoord 1)
    while (num === den) { den = ggd * floor(random(2, 5)); }
    
    let var1 = random(['x', 'a']);
    let var2 = random(['y', 'b']);
    let var3 = random(['z', 'c']);
    
    // Vereenvoudig met echte ggd van num en den
    let divisor = gcd(num, den);
    let finalNum = num / divisor;
    let finalDen = den / divisor;
    
    questionText = num + var1 + var2 + '/' + den + var1 + var3;
    
    if (finalNum === 1 && finalDen === 1) {
      answerText = var2 + '/' + var3;
    } else if (finalNum === 1) {
      answerText = var2 + '/' + finalDen + var3;
    } else if (finalDen === 1) {
      answerText = finalNum + var2 + '/' + var3;
    } else {
      answerText = finalNum + var2 + '/' + finalDen + var3;
    }
    
  } else if (type === 'simple_cancel') {
    // 12ab/4b = 3a
    // coef is altijd veelvoud van div zodat deling altijd geheel is
    let div = floor(random(2, 6));
    let coef = div * floor(random(2, 6));
    
    let var1 = random(['a', 'x', 'p']);
    let var2 = random(['b', 'y', 'q']);
    
    questionText = coef + var1 + var2 + '/' + div + var2;
    
    let result = coef / div;
    answerText = (result === 1) ? var1 : result + var1;
    
  } else if (type === 'add_term') {
    // 8x/4 + 2x = 4x
    // coef1 is altijd veelvoud van div zodat eerste term geheel getal geeft
    let div = floor(random(2, 5));
    let coef1 = div * floor(random(2, 5));
    let coef2 = floor(random(1, 5));
    
    let var1 = random(['a', 'x', 'y']);
    
    let term1 = coef1 / div;   // altijd geheel
    let result = term1 + coef2;
    
    questionText = coef1 + var1 + '/' + div + ' + ' + coef2 + var1;
    ...
    if (result === 1) {
      ...
      answerText = result + var1;
    }
    
  } else if (type === 'subtract_frac') {
    // 7/3a - 2/a = 1/3a
    // gemeenschappelijke noemer = den × var1
    // teller wordt: num1 - num2×den
    // daarna volledig vereenvoudigen
    let den = floor(random(2, 5));
    let num2 = floor(random(1, 3));
    
    // Zorg dat resultNum niet 0 is en niet negatief
    let num1 = num2 * den + floor(random(1, 5));  // altijd positief resultaat
    
    let var1 = random(['a', 'b', 'x']);
    
    let resultNum = num1 - (num2 * den);  // altijd > 0 door bovenstaande berekening
    
    questionText = num1 + '/' + den + var1 + ' - ' + num2 + '/' + var1;
    
    // Vereenvoudig resultNum/den volledig met ggd
    let divisor = gcd(resultNum, den);
    let simplNum = resultNum / divisor;
    let simplDen = den / divisor;
    
    if (simplDen === 1) {
      // Noemer valt weg: simplNum / var1
      answerText = (simplNum === 1 ? '1' : simplNum) + '/' + var1;
    } else if (simplNum === 1) {
      answerText = '1/' + simplDen + var1;
    } else {
      answerText = simplNum + '/' + simplDen + var1;
    }
    
  } else if (type === 'factor_out') {
    // (2p+2q)/(4p+4q) = 1/2
    // num minimaal 2 zodat vraag er netjes uitziet
    let num = floor(random(2, 5));
    let den = num * floor(random(2, 4));  // den is altijd veelvoud van num
    
    let var1 = random(['x', 'a', 'p']);
    let var2 = random(['y', 'b', 'q']);
    
    questionText = '(' + num + var1 + '+' + num + var2 + ')/(' + den + var1 + '+' + den + var2 + ')';
    
    // Vereenvoudig num/den met ggd
    let divisor = gcd(num, den);
    let finalNum = num / divisor;
    let finalDen = den / divisor;
    ...
    if (finalNum === 1 && finalDen === 1) {
      ...
      answerText = '1/' + finalDen;
    } else {
      answerText = finalNum + '/' + finalDen;
    }
  }
...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        try {
            let q = generateAlgebraicFractionQuestion();
            
            // Controleer op duplicaten ← hier aanpassen of je dubbelen wilt toestaan
            let isDuplicate = questions.some(existing => existing.answer === q.answer);
            if (isDuplicate) { i--; continue; }
            
            if (q && q.question && q.answer && 
                q.question.length > 0 && q.answer.length > 0) {
                questions.push({
                    text: q.question + ' = ?',
                    answer: q.answer
                });
            ...
            console.log('Error generating question:', e);
            i--;
        }
    }
    ...
    // Plaats vraagblokken (boven, rows 1-2)
    let questionIndex = 0;
    ...
    // Plaats antwoordblokken (onder, rows 3-4)
    let answerIndex = 0;
...
function drawBlock(block) {
  push();
    ...
    if (block.text.includes('/')) {
        ...
        let hasLongFraction = false;
        
        // Kies tekstgrootte op basis van breuklengte
        let fracSize   = hasLongFraction ? 11 : 15;  // ← tekstgrootte teller/noemer
        let otherSize  = hasLongFraction ? 11 : 15;  // ← tekstgrootte rest (=, ?, +, -)
        let fracOffset = hasLongFraction ?  8 : 11;  // ← verticale afstand teller/noemer van streep
        let fracMargin = hasLongFraction ?  2 :  4;  // ← marge aan weerszijden breukstreep
        let partGap    = hasLongFraction ?  5 :  8;  // ← ruimte tussen delen
        
        // Meet totale breedte EERST voor correcte centrering
        textSize(fracSize);
        let totalW = 0;
        for (let part of parts) {
            ...
            if (part.includes('/')) {
                let [n, d] = part.split('/');
                textSize(fracSize);
                let lh = max(textWidth(n), textWidth(d)) / 2 + fracMargin;
                totalW += lh * 2 + partGap;
            } else if (part === '+' || part === '-' || part === '=' || part === '?') {
                textSize(otherSize);
                totalW += textWidth(part) + partGap;
            } else {
                textSize(otherSize);
                totalW += textWidth(part) + partGap;
            }
        ...
        // Startpositie zodat alles gecentreerd staat
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (part.includes('/')) {
                ...
                let numW = textWidth(num);
                let denW = textWidth(den);
                let lineHalf = max(numW, denW) / 2 + fracMargin;
                
                // Teller
                text(num, startX + lineHalf, baseY - fracOffset);
                
                // Breukstreep
                stroke(0);
                strokeWeight(1.5);
                line(startX, baseY, startX + lineHalf * 2, baseY);
                noStroke();
                
                // Noemer
                fill(0);
                text(den, startX + lineHalf, baseY + fracOffset);
                
                startX += lineHalf * 2 + partGap;
                
            } else if (part === '+' || part === '-') {
                textSize(otherSize);
                fill(0);
                text(part, startX + textWidth(part) / 2, baseY);
                startX += textWidth(part) + partGap;
                
            } else if (part === '=') {
                textSize(otherSize);
                fill(0);
                text('=', startX + textWidth('=') / 2, baseY);
                startX += textWidth('=') + partGap;
        ...
        // Tekst zonder breuk — gecentreerd in blokje
        textSize(15);  // ← lettergrootte antwoordblokjes
        fill(0);
